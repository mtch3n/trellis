...
func TestKnowledgeMetadataEdit(t *testing.T) {
	...
	doc, err = c.EditKnowledgeFields(t.Context(), p.ID, doc.Slug, KnowledgeEdit{Template: new("decision"), Private: new(true), Tags: new([]string{"new"}), Labels: new([]string{"reviewed"}), IfVersion: &doc.Version})
	if err != nil {
		t.Fatal(err)
	}
	if doc.Template != "decision" || !doc.Private || !slices.Equal(doc.Tags, []string{"new"}) || !slices.Equal(doc.Labels, []string{"reviewed"}) || doc.Recap != nil {
	...
	if fm.Template != "decision" || !fm.Private || !slices.Equal(fm.Tags, doc.Tags) || !slices.Equal(fm.Labels, doc.Labels) {
	...
	if err != nil || !doc.Private || doc.Template != "decision" || len(doc.Tags) != 1 || len(doc.Labels) != 1 {
