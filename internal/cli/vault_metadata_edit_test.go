...
func showEntry(t *testing.T, ref string) core.Entry {
	t.Helper()
	var got core.Entry
	if err := json.Unmarshal([]byte(runCmd(t, "vault", "show", ref, "--json")), &got); err != nil {
		t.Fatal(err)
	}
	return got
}

func TestVaultEditMetadataFlags(t *testing.T) {
	projectEnv(t)
	runCmd(t, "label", "new", "reviewed", "--description", "Reviewed")
	runCmd(t, "vault", "new", "--title", "Metadata")
	runCmd(t, "vault", "edit", "metadata", "--template", "research", "--private=true", "--tag", "one", "--tag", "two", "--label", "reviewed", "--if-version", "1")
...
func TestVaultEditPrivateFlagAcceptsAnyBoolSpelling(t *testing.T) {
	projectEnv(t)
	runCmd(t, "vault", "new", "--title", "Secret", "--private")
	v := showEntry(t, "secret").Version
	runCmd(t, "vault", "edit", "secret", "--private", "True", "--if-version", strconv.FormatInt(v, 10))
	if got := showEntry(t, "secret"); !got.Private {
		t.Errorf("--private True made the entry public: %+v", got)
	}
}

func TestVaultEditPrivateFlagRejectsGarbage(t *testing.T) {
	projectEnv(t)
	runCmd(t, "vault", "new", "--title", "Secret2")
	v := showEntry(t, "secret2").Version
	_, err := runCmdErr(t, "vault", "edit", "secret2", "--private", "yes", "--if-version", strconv.FormatInt(v, 10))
	if err == nil {
...
func TestVaultEditReplacesAndClearsTagsAndLabels(t *testing.T) {
	projectEnv(t)
	runCmd(t, "label", "new", "priority", "--description", "Priority")
	runCmd(t, "label", "new", "reviewed", "--description", "Reviewed")
	runCmd(t, "vault", "new", "--title", "Tagged")
	edit := func(args ...string) core.Entry {
		t.Helper()
		v := showEntry(t, "tagged").Version
		runCmd(t, append(append([]string{"vault", "edit", "tagged"}, args...), "--if-version", strconv.FormatInt(v, 10))...)
...
// vault edit --set writes a template field.
func TestVaultEditSetsAField(t *testing.T) {
	projectEnv(t)
	runCmd(t, "vault", "new", "--title", "Owned")
	runCmd(t, "vault", "edit", "owned", "--set", "owner=alice", "--if-version", "1")
