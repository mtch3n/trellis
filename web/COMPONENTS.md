...
| ArtifactList | `Button`, `ArtifactPreview`, lucide | An entry's artifacts in the facts column. A row opens the preview; an archive row downloads; a name with no file behind it reads as a broken reference, not an error |
