...
| MessageScroller | `@/components/ui/message-scroller` | The chat's conversation: follows a streaming reply, holds the user's turn in view, and offers a jump back to the latest |
| Message | `@/components/ui/message` | One row of the chat, aligned to the side of who wrote it |
| Bubble | `@/components/ui/bubble` | The surface of the person's own message in the chat. Replies have none: they are read, not answered to |
| Marker | `@/components/ui/marker` | A status line in the chat, such as the shimmer while a reply has not started |
| Sheet | `@/components/ui/sheet` | Installed with the chat primitives. The chat itself is docked rather than a sheet, so the board stays visible and live beside it |
...
| Component | Wraps | Why |
|-----------|-------|-----|
| AppShell | `ProjectSwitcher`, `BoardSwitcher`, `Lamp`, `ThemeToggle`, `IconButton`, `GuardedLink` | The persistent chrome: the mark, the project scope control, the board switcher (when multiple boards exist), the three sections (Overview, Board, Vault), connection status (shown only when there is something to say: Live, Stale or Offline), and Settings as the right-most control, a gear on a muted surface while settings are open. Every way out of a screen goes through the navigation guard. No rule under it at rest; a hairline appears once content scrolls beneath it. Project is a scope control rather than a destination, per PRODUCT.md principle 3: one project is the working scope and switching is a deliberate act. The current section is marked by a foreground rule that grows from the centre, never by the claimed accent |
...
| HistoryDialog | `Dialog`, `Button`, `Empty`, `Skeleton`, `IconButton` | The revisions of a card or an entry, and what one save changed: the list on the left, newest first, and the server's unified diff for the chosen version, drawn by `PatchView`. History means revisions, so a card's comments and events stay in its Timeline. Version numbers have gaps, because a move bumps a card's version without keeping a revision, so the list is what the server kept rather than a count |
...
| PatchView | `diff` (jsdiff) `parsePatch`, `DiffView`'s rows | A unified diff the server made, drawn with the same rows as `DiffView`, so a revision's diff and a diff made in the browser read alike. File headers are dropped, since the dialog names the subject; each hunk opens with the lines it covers |
...
| BoardMenu | `DropdownMenu`, `Dialog`, `Field`, `Input`, `Switch`, `ConfirmDialog`, `RefusalAlert`, `Tooltip` | A board's own settings behind one button in its header: another board, renaming this one, making it the board the project opens on, the project's labels, and deleting it. Renaming keeps the board's slug, because a `.trellis` marker names the slug and a rename must not strand a repository; deleting a board that still holds cards says how many, and a project always keeps one board |
| ColumnMenu | `DropdownMenu`, `Button`, ColumnForms | One column's settings, on its header and shown when the column is pointed at, since the cards are what a reader is there for: rename, move left or right, add one after it, delete it. A column's place is "after this one", the way the CLI takes it and the way a strip reads, so moving is left and right rather than a number |
| AddColumnButton | `IconButton`, ColumnForms | Adding the first column. A board started without the usual four has nowhere to put a card until it has one, so the empty strip offers this directly |
| ColumnForms | `Dialog`, `Field`, `Input`, `Select`, `Switch`, `RefusalAlert` | The three forms a column needs — add, rename, delete — kept in one place for whichever control opens them, so a menu and a button share them. Deleting a column with cards in it asks where the cards go, because the cards are the work and the column is only where they sit |
| LabelsDialog | `Dialog`, `Field`, `Input`, `Select`, `IconButton`, `ConfirmDialog`, `RefusalAlert` | The project's label vocabulary: what each label means, a new one (which has to say what it means, because a label nobody can define is a label nobody uses the same way), merging two words that turned out to be one thing, and removing one. Deleting a label cards still carry is refused by the server, which says how many |
| EntryMenu | `DropdownMenu`, `Tooltip`, `ConfirmDialog`, `Button`, `toast` | An entry's secondary actions behind one button in its action row, as `CardMenu` holds a card's: copy a link, History, pin it for later sessions, and the lifecycle acts. Which of those appear follows the entry — a project entry can be promoted, a global one demoted or verified — and Delete comes last, confirmed, saying the file goes with the row |
| LifecycleDialog | `Dialog`, `Field`, `Input`, `Textarea`, `RefusalAlert`, `Button`, `Spinner` | The acts that move an entry between a project and the global vault, and pinning. The CLI refuses promote, demote and verify to an agent and points at a person, so this is where they happen, and it asks for what the CLI's operator has to think about: the entry's name retyped (with or without its folders) and why. A refusal is shown in the form, because the form is where it is corrected |
...
| VaultLayout | `Resizable`, `react-resizable-panels` | The vault's frame: navigator and entry side by side with a draggable divider (14rem to 36rem, 18rem by default), remembered per browser. The navigator keeps its pixel width when the window changes, and collapses from its toggle or by dragging the divider past its narrowest, reopening at the width it had; collapsed, it is inert. The toggle eases it open and shut over 200ms on `ease-settle`, the tree clipped at its narrowest width and fading, the same motion as the facts and the outline; a drag has no transition. The divider has no colour at rest and shows when pointed at. Below lg the two stack, and a hidden navigator is simply not rendered |
...
| VaultStart | `Empty`, `Button`, `Link`, `@/lib/recent` | What the vault shows with no entry open: somewhere to go next rather than a count. The entries this browser opened last, the ones edited last, and the pinned ones, each as a short list of links with a time beside it; an empty list is left out, and an empty vault says so. A slug nothing answers to is said in its heading |
| EntryOutline | none (registry search below), `@/lib/outline` | The open entry's headings as a rail beside it, fumadocs-style: a faint line with a thumb that spans every section showing in the pane, so it tracks what is being read rather than jumping between single headings. A click scrolls to the heading and leaves the address alone. Shown only from xl, and from 2xl when the facts take the room, and only when the entry has two headings or more. It and the facts column stay mounted from xl and ease width and opacity when shown or hidden, so the entry slides over rather than jumping |
| VaultNav | `InputGroup`, `Collapsible`, `DropdownMenu`, `Tooltip`, `IconButton`, `NavLink`, lucide, `@/lib/vault-tree` | The vault as a file explorer. Entries are markdown files and the scopes are directories, so the tree shows that structure: the global vault and the project as top folders, subfolders wherever a slug has slashes, one line per file with a lock on private ones. Two rows head it: the filter on its own, then the tools. They start a new entry, filter (which opens every folder), sorts files within their folders (folders always first), and collapses or expands everything. A project folder's row offers a new entry inside it, a plain plus over its count. Folders remember whether they were open, and the folders around the open entry open themselves when it changes. The arrow keys walk the rows. Nested contents hang from a faint guide under the folder's chevron. The selection is one surface that slides between rows. Takes the graph dock as a slot |
...
| FilterMenu | VaultNav | What the tree shows and how it groups: the templates and provenances kept, and whether files sit in the directories their slugs spell out or under the template they follow. Nothing chosen means everything, so the menu opens saying nothing rather than listing the vault back. Grouping by template is how a reader asks "what decisions do we have?" without a decisions directory, which the vault-paths design rules out; a template is never turned into a directory |
...
| SettingsTemplates | `Table`, `Badge`, `Dialog`, `PageHeader` | The entry templates on this machine, shared by every project: name, strict or advisory, section count, and a Built-in badge. A row opens the file; New template names one and opens it |
...
| SettingsAgents | `Table`, `Badge`, `Lamp`, `Empty`, `PageHeader` | Who has been working here and what they claim — the triage view for a contended card: an agent quiet for a while is one whose claim can be taken, and an expired claim needs no taking at all. An actor with a claim but no agent row still appears, because the claim is what holds a card; otherwise a card would be claimed by nobody the page can name |
| SettingsDiagnostics | `MetaFacts`, `Lamp`, `Separator`, `Button`, `Spinner`, `PageHeader` | What `trellis doctor` says, in the browser: each check with the command that fixes it, a warning being what will surprise someone later and a failure what is broken now. The version sits below, with the release check as a button rather than a page load because checking asks GitHub, and a newer release is reported as the command to run: a web page must not be able to replace the binary serving it |
| ChatPanel | `MessageScroller`, `Message`, `Bubble`, `Marker`, `InputGroup`, `Select`, `EffortSelect`, `Empty`, `Alert`, `IconButton` | The chat, docked at the right edge beside whatever page is open, so the board stays in view while the agent reads and changes it; on a wide screen the page makes room rather than hiding under it. Opened from the shell or with Ctrl . and remembered across pages. It talks to one provider at a time, with the effort chosen beside it. The conversation belongs to the project, survives moving between pages, and is kept in the browser |
| AssistantMessage | `Message`, `MarkdownContent`, `Reasoning`, `ToolCall` | A reply in the order it happened: reasoning, tool calls, words, and which provider gave it |
| Reasoning | `Collapsible` | The model's reasoning, folded away unless asked for. It shimmers while it runs |
| ToolCall | `Collapsible`, `Spinner`, `GuardedLink` | One tool call as a single line: what it did, on what, and how it went. Opening it shows the exact input and answer. A card the call touched is a link, so a change the agent made is one step from being checked |
| EffortSelect | `Select` | How hard the model reasons. The same control in a provider's settings, where it sets the default, and in the chat, where it overrides it |
| SettingsProviders | `Table`, `Badge`, `Button`, `DropdownMenu`, `Empty`, `Alert`, `ConfirmDialog`, `PageHeader` | The providers the chat can use: APIs the daemon forwards to with the key it holds, and agent programs it runs here. A row opens its editor; Test stays on the row, and making it the default or removing it sit behind its menu. The key is written once and never shown again; the table says only whether one is set, and flags a provider that needs one and has none |
| ProviderDialog | `Dialog`, `FieldGroup`, `Field`, `Select`, `Input`, `Alert`, `Spinner` | Adding or changing one provider. The kind decides the fields: an API asks for base URL, model, key and the variable that overrides the key; a local agent asks for its command, model and working directory. A new provider's ID is derived from its name and cannot change later, since config.yaml names it by that |
| SettingsVector | `Select`, `Separator`, `Button`, `Spinner`, `PageHeader` | One project's vector index: whether it is configured, how much of the vault is in it, and the four things that can be done to it — rebuild, prune, reindex, compact. Off is a state rather than a fault, since text search carries on without it, so the page says so and offers nothing to run. The index belongs to a project, so the project is picked rather than assumed |
| SettingsMaintenance | `Select`, `Separator`, `ConfirmDialog`, `PageHeader` | What the database weighs, as one line, and the ways to trim it as a plain list: prune events and invocations older than a chosen age, trim revisions to `history.keep`, remove leftover revisions, compact, back up now, and prune old backups. Every deletion asks first and reports how much went; compacting and backing up only rewrite or add, so they run at once |
| MaintenanceRow | SettingsMaintenance | One task: what it does on the left, how to run it on the right |
| KeepSelect | SettingsMaintenance | How many backups a prune leaves. Pruning only ever considers the backups Trellis named, so the rest of that directory is not its business |
...
| MarkdownContent | `react-markdown`, `remark-gfm`, `rehype-sanitize`, `@/lib/wikilinks` | Safely renders stored Markdown with the shared typeset stylesheet; sanitization keeps raw HTML from becoming an injection surface. The prose measure lives in `.typeset-notes`, not in a className. Given `wikilinks`, `[[targets]]` in the text become links: the vault's files spell links the way Obsidian does, so reading one here follows it. Without it a wikilink stays the text it is, which is what a comment or a diff wants. Given `anchors`, headings get GitHub-style ids from `rehype-slug`, run after sanitizing so the ids stay unprefixed, which is what `EntryOutline` points at. Given `citations`, GFM footnote references become `CiteMark` chips |
| CiteMark | `Badge`, `Popover`, `@/lib/citations` | A footnote reference drawn as a citation: a square chip naming the note's source, the host of its first web link or its opening words. Pointing at it or focusing it opens a card with the source, the title the note's link was given, and the rest of the note as an excerpt: `[^1]: [Title](https://…) excerpt`. Nothing is fetched; the card shows only what the entry wrote. With a link it opens it in a new tab; without one it scrolls to the note at the foot of the text. The markdown stays plain footnotes (`claim[^1]`, `[^1]: https://…`), so the file reads the same anywhere, and only http and https links are ever opened |
| WikiAnchor | MarkdownContent | An anchor the wikilink plugin made: a router link when the entry exists, and a dashed, muted mark when it does not, because a stub is a loose end and not an alarm. Every other anchor is left as the markdown wrote it |
| TrellisMark | none (the brand path) | The Trellis mark, the same path data as `brand/logo/mark.svg`, so the shape stays the brand's wherever it is drawn. The kit pairs charcoal-on-ivory with an inverse for dark surfaces; the shell is dark by default and light by choice, so it draws the silhouette in `currentColor` — the one-colour use of the mark — and the chrome stays achromatic like every other glyph in it |
...
**EntryOutline.** `pnpm dlx shadcn@latest search @shadcn -q "table of contents"`,
`-q "outline"` and `-q "scrollspy"` returned *No items found*; `-q "toc"` returned only
sidebar and sign-in blocks. Composition cannot produce the behaviour: the rail tracks
which sections are on screen as the pane scrolls and sizes its thumb to span them, which
no primitive measures. `fumadocs-core/toc` does this headlessly but brings shiki, zod and
a framework adapter with it. EntryOutline owns the scroll tracking and the thumb; the
heading ids come from `rehype-slug` and are read back from the page by `useOutline`.
...
| SearchPage | Search across both halves of the product, cards and vault entries, in every project by default, because not knowing which project holds the answer is the reason to search. The options are the CLI's: the method (the words as written, the meaning, or both), the project, a label, and how many hits. A hit opens what it names |
| EventLogPage | The event log: every action across every project, newest first, at `/event-log` |
| SettingsPage | The machine's settings, and what it is doing, at `/settings/:section`, not a project's, so the shell carries no project. A section menu on the left and the open section on the right, within a 5xl container; on a narrow screen the menu becomes a row above. The menu is plain words, the current one on a muted surface, and its links go through the navigation guard. Sections route inside the page, so the shell stays mounted |
| GraphPage | The graph walked from one card, entry or artifact at `/p/:key/graph/:entity`: how far out, along which kinds of link, and in which direction. The vault's own graph answers "what is connected to what"; this answers "XPSCTL-12 is blocked — by what, and is that thing itself blocked?", which one hop cannot see, and walking inbound turns it into "what breaks if I change this". The controls live in the URL, so a walk worth showing someone is a link |
| HealthPage | The vault's own state at `/p/:key/health`, under the Vault section: the counts, the diagnostics behind them, possible duplicates, the entries nothing reads any more, the nominations waiting on a person, and whether recall is paying off. Detection is free and runs on demand; every action on it is a person's, so promoting from here asks for the entry's name and a reason |
...
| SearchOption | SearchPage | One search option as a labelled control: the method, the project scope, the label, and how many hits. Method and label only appear where they mean something — a label belongs to one project, so the filter follows the scope |
...
- A board's own shape — the board, its columns, the project's labels — is written through `boardActions` (`@/lib/board-actions`). Like `vaultActions` it answers with the refusal, because every one of those writes starts from a form that has to show why it was refused.

- Every write a card supports lives in `cardActions` (`@/lib/card-actions`), which the board and the card page share: the fetch, the toast on failure, and the caller's own refresh afterwards. A new card action belongs there, not in a page. An entry's writes live beside it in `vaultActions` (`@/lib/vault-actions`), including the lifecycle acts, which answer with the server's refusal rather than a toast because a dialog has to show it.
