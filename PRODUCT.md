...
2. **Watching agents work.** Live, during a session, to see cards move and claims
...
A local-first kanban board and vault for work that is executed by AI agents. One
Go binary ships the CLI, an optional daemon, and the embedded web UI. Success is
that a human returning to a project can re-establish what is happening without
re-reading transcripts, and that an agent can record progress and what it learns,
so both survive the end of its session.

## Positioning

The board's primary caller is an agent, not a human. Cards, claims, comments and
entries are written by agents through the CLI; the web UI is the human's window
onto that work rather than a place they maintain a backlog by hand. This
...
- `internal/resolve` maps a working directory to a project via `--project`,
  `TRELLIS_PROJECT`, then the nearest .trellis marker.
...
- Boards, columns, and cards carrying priority, an optimistic-concurrency
  `version`, and an expiring claim. Cards accumulate comments and events, shown
  together as a timeline.
- Claims can be stolen, with a reason recorded.
- Vault entries with wikilinks, labels, and a link graph. A wikilink whose
  target does not resolve is a stub, not an error.
- Search across cards and entries: FTS5, vector, and hybrid.
- Markdown files are the source of truth for an entry's body. SQLite holds
...
- `/TRELLIS/vault/pain-point-analysis-sept-2026` — derived from 20,287 prompts and
  1,810 session transcripts, Feb–Sep 2026. Read it before re-analyzing; the entry
...
- The same entry records that scope creep within a session, not cross-session
  context loss, is the top user frustration — and that Trellis does not address
  it. Future work must not claim otherwise.
- That entry also flags a fabricated "standing preferences" table produced by
