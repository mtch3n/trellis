...
An agent may create and update cards and knowledge entries in the situations
listed below, without asking first. This is a standing authorization and it is
the point of this skill: a board no one may write to stays empty.

It does not extend to: claiming work the user did not ask you to take, moving
another actor's cards, archiving, deleting, or `knowledge nominate`. Those need
the user. Reading the board is always allowed and needs no authorization.

## Triggers

| Situation | Do this |
|---|---|
| Task has 3+ steps, or will span more than one sitting | One card per step; `card block` for real dependencies |
...
| Finding the next session would re-derive | `knowledge new`; pin it only if not knowing it causes a wrong action |
| Another agent needs to pick this up | Card with enough context to start cold, then `card release` |
| User says "track this", "for later", "don't forget" | Take it literally — card or knowledge entry, now |
