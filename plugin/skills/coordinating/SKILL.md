...
comment and coordinate, rather than continuing to write to work someone else owns.
...
`renew`, `comment`, `edit`, `move`. A worker that uses its suffix inconsistently
...
comment is worse than no card: it claims the work exists but not where it stands.
